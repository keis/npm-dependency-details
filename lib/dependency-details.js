var url = require('url')
  ...
  , parallel = require('./thunk-parallel')
  , each = require('./thunk-each')
  , eachSync = require('util-each')
...
function packageNames(packageInfo) {
    return union(Object.keys(packageInfo.installed),
...
function allPackageNames(packageInfo) {
    ...
    function inner(packageInfo) {
        ...
        eachSync(packageInfo.installed, function (val, key) {
...
function investigate(options, path) {
    ...
    var npm = {},
        config = npm.config = options.config;
